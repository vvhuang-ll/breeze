...
HOST_VIP=`cat /opt/wise2c/tmp/kubernetes/kubeadm.conf | grep -A 1 SAN | tail -1 | awk '{print $2}'`

set -e

# Get apiserver address
service_cidr=`cat /opt/wise2c/tmp/kubernetes/service_cidr.txt`
